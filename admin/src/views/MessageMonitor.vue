<template>
  <div class="message-monitor-container">
    <div class="page-header">
      ...
      <p class="page-desc">总管理后台消息监控功能</p>
    </div>
    ...
    <el-dialog v-model="detailDialogVisible" title="消息详情" width="700px">
      <el-descriptions :column="1" border v-if="currentMessage">
        <el-descriptions-item label="发送者">
          <div class="user-info">
            ...
            <div style="margin-left: 12px">
              <div class="user-name">{{ currentMessage.sender.name }}</div>
              <div class="user-id">ID: {{ currentMessage.sender.id }}</div>
            </div>
          </div>
        </el-descriptions-item>
        <el-descriptions-item label="接收者">
          <div class="user-info">
            <el-avatar :size="50" :src="currentMessage.receiver.avatar" />
            <div style="margin-left: 12px">
              <div class="user-name">{{ currentMessage.receiver.name }}</div>
              <div class="user-id">ID: {{ currentMessage.receiver.id }}</div>
            </div>
          </div>
        </el-descriptions-item>
        ...
        <el-descriptions-item label="标记时间" v-if="currentMessage.readAt">
...
const handleMarkAsRead = (row) => {
  row.status = 'read'
  row.readAt = new Date().toLocaleString('zh-CN')
...
const handleBatchMarkAsRead = async () => {
  ...
  if (unreadMessages.length === 0) {
    ElMessage.warning('所选消息已全部已读')
    return
  }

  try {
    await ElMessageBox.confirm(
      `确定要标记选中的 ${unreadMessages.length} 条消息为已读吗?`,
      '提示',
      {
    ...
    ElMessage.success(`成功标记 ${unreadMessages.length} 条消息为已读`)
